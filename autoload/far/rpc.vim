...
function! far#rpc#invoke(imports, evalstr) abort "{{{
    ...
    call add(cmd, ' results = '.a:evalstr)

    " py3eval will raise an error if matched lines in result json contains null bytes (cf vim does not know how to handle them)
    " This can happen if some of the matching files are not POSIX compliant and their last line (which is matched) does not end with a eol \x0a character (e.g. when generated by vscode..)
    call add(cmd, ' def remove_null_bytes(data):')
    call add(cmd, '  if isinstance(data, dict):')
    call add(cmd, '   return {k: remove_null_bytes(v) for k, v in data.items()}')
    call add(cmd, '  elif isinstance(data, list):')
    call add(cmd, '   return [remove_null_bytes(item) for item in data]')
    call add(cmd, '  elif isinstance(data, str):')
    " to escape nested 's: double them
    call add(cmd, '   return data.replace(''\x00'', '''')')
    call add(cmd, '  else:')
    call add(cmd, '   return data')
    call add(cmd, '   return data')
    call add(cmd, ' return remove_null_bytes(results)')
